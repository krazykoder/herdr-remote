...
| # | Item | Blocks | Owner |
|---|---|---|---|
| 1 | ~~**P3 bracketed-paste preflight is INCONCLUSIVE.**~~ **Resolved PASS** on 2026-08-09 against a visible focused pane; see dev-notes §6.4. The earlier empty reads were the reading method, not the sending. | — | done |
...
| 3 | ~~**`start.sh` misreports startup failures.**~~ **Fixed** before P2. It printed "Check if port 8375 is in use" for *any* death within 2s, including a fail-closed config `exit(1)`. It now preflights the port with `lsof` (naming the offending process) and otherwise reports the relay's real exit status. A second defect surfaced while fixing it: the `EXIT` trap ended in `exit 0`, so `start.sh` returned success on every startup failure. | — | done |
