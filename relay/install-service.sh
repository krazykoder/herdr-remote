...
# Dual listeners. Carried through rather than prompted for: unset external port means no external
# listener, which is the safe default. Dropping them here would silently undo a dual-listener
# setup on the next re-run and leave the tunnel pointed at the LAN port.
#   .workflow/02_architecture/2026-08-09_dual_listener_access.md
HERDR_EXTERNAL_PORT=${HERDR_EXTERNAL_PORT:-}
...
if [ "$TUNNEL_MODE" = "named-external" ]; then
    ...
    # A tunnel must terminate on the token-required listener, never on the LAN one. Pointing it at
    # the LAN port publishes whatever that port's policy is, and with HERDR_LAN_OPEN=1 that is no
    # policy at all — an unauthenticated relay on the public internet. start.sh has refused this
    # since the dual listener landed; the installed service ran with no such guard.
    #   .workflow/02_architecture/2026-08-09_dual_listener_access.md
    TUNNEL_TARGET_PORT="${HERDR_EXTERNAL_PORT:-$WS_PORT}"
    if [ -z "${HERDR_EXTERNAL_PORT:-}" ]; then
        if [ "${HERDR_LAN_OPEN:-}" = "1" ]; then
            echo "Error: HERDR_LAN_OPEN=1 with no HERDR_EXTERNAL_PORT — refusing to install a"
            echo "       tunnel to a token-free listener. Set HERDR_EXTERNAL_PORT (and a relay"
            echo "       token) in $CONFIG_FILE, or choose tunnel mode 'none'."
            exit 1
        fi
        if [ -z "${HERDR_RELAY_TOKEN:-}" ]; then
            echo "Error: a tunnel with no relay token would publish this machine's agents to"
            echo "       anyone with the URL. Re-run and accept the access token, or choose"
            echo "       tunnel mode 'none'."
            exit 1
        fi
    fi
    echo "  Tunnel terminates on 127.0.0.1:$TUNNEL_TARGET_PORT"
...
ingress:
  - hostname: $TUNNEL_HOSTNAME
    service: http://127.0.0.1:$TUNNEL_TARGET_PORT
  - service: http_status:404
EOF
        TUNNEL_ARGS="tunnel --config $CF_CONFIG run $TUNNEL_NAME"
        echo "  Tunnel config: $CF_CONFIG"
    else
        TUNNEL_ARGS="tunnel --url http://127.0.0.1:$TUNNEL_TARGET_PORT"
    fi
