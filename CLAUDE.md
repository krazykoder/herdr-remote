...
| `relay/git_probe.py` | Where the work landed: the branch a pane's cwd is on and the commits since its last recorded turn. Read-only git, run at turn end rather than per poll | Python (stdlib) |
...
| `HERDR_GIT_TRACK` | `0` stops the record asking where the work landed. On with `HERDR_CONV_LOG`, and nothing at all without it: every turn then also carries the branch of the pane's cwd, the commit it was read at, and the commits made since that directory's previous turn. Read-only git commands (`rev-parse`, `log`), run at turn end and never per poll |
