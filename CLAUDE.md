...
| `relay/conv_query.py` | Read-only query over that record. Also a CLI, so an agent can read it from its own shell. A row the writer marked as a copy of another row — the same message read twice, when a pane re-wrapped between two reads — is left out of every answer unless `--dupes` asks for it: the record keeps everything it detected, and a copy is still not a second thing that was said | Python (stdlib) |
| `relay/agent_configs.py` | Which environments a session may start under, and the security boundary of the feature: a *provider* is file-only and names the endpoint, the config directory and which keystore ids may supply a credential; an *alias* lives in shared state, is edited from the app, and can only choose among what that file already authorised. Builds the export line typed at the pane, which fetches the key with `secret <id>` in the pane's own shell — so a key value never reaches the relay's memory, environment or logs. The started agent inherits it and must still be trusted with it | Python (stdlib) |
...
| `relay/prompts/*.md` | Every word the arbitrator reads — the opening brief, the two instruction styles, the roster announcement, the resume notes. Markdown so they can be curated by hand between sessions; `## name` splits a file into sections and `$name` is substituted. Re-read on every send, so an edit takes with no restart; a broken edit keeps the last copy that loaded, and a broken *boot* fails the relay rather than a session | Markdown |
| `relay/arb_prompts.py` | The loader for those files, and the one place they are named | Python (stdlib) |
...
| `relay/backup-state.sh` | The copy that is not an undo. `sqlite3 .backup` of both databases with the relay live, plus the drop boxes, the config directory without `secrets.env`, and the push subscriptions — into a timestamped directory under `.herdr-remote/backups/`, pruned to the last `HERDR_BACKUP_KEEP` (14). Run by the relay itself every `HERDR_BACKUP_HOURS`; it can also be run by hand or exposed through the ops bot's command registry. No launchd job or cron line: a system-level schedule is a second thing to install, a second thing to remember to remove, and on macOS a permission prompt about a program the person did not start. Never by a test relay, which would copy the running install's data into the checkout it is pretending to be. The shared store's 200 revisions per document are an undo with a window of about two hours at the rate the conversation index moves, which is not the same thing | Bash |
...
| `web/src/agent_configs.js` | The launcher's Agent configs section: an alias per row banded by harness, the view/edit dialog, and the effective command line the spawn will run — with the key as a `$(secret <id>)` call, so the row is both safe to read and pasteable into a terminal | JavaScript |
...
| `HERDR_PROJECTS_FILE` | Absolute path to the Projects config JSON (unset = Projects disabled). Re-read while the relay runs: a stat per root each poll, a listing only when one moved, and a `projects` broadcast when the roster actually changes. Boot still fails closed on a bad file; a bad *edit* keeps the last roster that parsed, because taking a running relay down would cost every client its session to punish a text editor. An entry with `"children": true` is a **root**: each directory one level under it is a Project in its own right, found by listing rather than written down — which is how an agent gets a place of its own without anything editing the file. Depth one, so a directory inside a child is never a candidate; dotdirs, files and symlinks are skipped, the last because a symlink is the one way the filesystem could name a path outside the root — and asked again at every spawn, on the resolved path, because the directory that was listed can be swapped for a symlink before anyone starts anything in it. `"marker": ".git"` narrows a root that holds other things to the directories containing that file. `"container": true` says a root is a place projects live rather than one to work in: it is still scanned, still takes new children, and still names any pane sitting directly in it, but it is never offered as somewhere to start. Meaningless without `children`, and refused rather than ignored. A child's id is its root's plus its folded directory name (`common-charts-ts`), its label is the real name, and it carries `parent` on the wire — the one thing that says a row was derived rather than authorised. Local-only: scanning a remote root is an ssh round trip per poll, and it is refused rather than silently never done |
...
| `HERDR_AGENT_CONFIGS` | Path to the providers file (default `~/.config/herdr-remote/agent-configs.json`). Absent = no *custom* providers, which is the default state: the stock harnesses (agy, claude, codex) are offered either way, as providers that name no endpoint and no secret and put the model on argv as `--model <name>` — so a session on a non-default model is a named alias and badges under that name. A file provider names a base URL, a config directory, which keystore ids may supply a key — fetched by the pane's shell with `secret <id>`, never by the relay — and which variable carries the model; a file entry reusing a stock id replaces it. File-only by design: a client can name an alias but never an endpoint or a secret. Malformed is fatal — the relay says which field and exits |
| `SECRETS_FILE` | The keystore the `secret` shell function reads (default `~/.bash_secrets`). The relay reads it too, for key *names* only, so an agent config's row can say whether the id it names exists. Values are never read by the relay — the pane's own shell fetches them at spawn time. The started agent inherits the exported value and must be trusted with it |
| `HERDR_BACKUP_HOURS` | How often the relay copies its own state (default 24; `0` off). Checked every ten minutes against the newest backup's timestamp rather than slept for a day, so a laptop closed at 03:00 backs up when it opens instead of missing its turn, and a relay restarted six times a day still backs up once. `HERDR_BACKUP_KEEP` (14) is how many are kept, `HERDR_BACKUP_DIR` where they go |
