...
| `HERDR_AGENT_CONFIGS` | Path to the providers file (default `~/.config/herdr-remote/agent-configs.json`). Absent = the feature is off and nothing is offered. A provider names a base URL, a config directory, which of *this relay's* environment variables may supply a key, and which variable carries the model. File-only by design: a client can name an alias but never an endpoint or a secret. Malformed is fatal — the relay says which field and exits |
