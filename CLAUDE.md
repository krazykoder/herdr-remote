...
| `SECRETS_FILE` | The keystore the `secret` shell function reads (default `~/.bash_secrets`). The relay reads it too, for key *names* only, so an agent config's row can say whether the id it names exists. Values are never read here — the pane's own shell fetches them at spawn time, which is what keeps a key out of the relay's environment and out of anything an agent could echo |
